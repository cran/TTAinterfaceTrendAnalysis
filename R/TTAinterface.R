...
#_______________________________________________________________________________________________________________________________________Onglet Raw Data
 ...
 getfile <- function(inipath) {
     ...
     Envir$Text <- tk2label(Envir$rawdata,text= paste("Current active file: ", Envir$Name.split, empty))                                                   # texte montrant le fichier actif
     ...
     Envir$Text2 <- tk2label(Envir$rawdata, text= paste("Current save directory: ", Envir$save.WD, empty))       # texte montrant chemin de sauvegarde
     ...
     HELP1.but <- tk2button(Envir$HelpFrame, image=imgHelp, text=" Help ", compound="right", command=function() { Aide1() })                       # fichier txt a aller chercher
...
#_ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _Liste de category
     ...
     #cb <- tkcheckbutton(Envir$Select)                                                # check case pour agreger les SERIES (obsolete)
     #cbValue <- tclVar("1")
...
#_ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _Liste des parametres
	...
	RightClick <- function(x, y) { 
        ...
        tkbind(liste3, "<Button-1>", RightClick)	}
...
#_ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ Sliders salinite et profondeur
     ...
     if (any(colnames(Envir$Data) == "Salinity") & any(!is.na(Envir$Data$Salinity))==TRUE) {                                                     # si une colonne S est presente
        if (class(Envir$Data$Salinity) == "numeric") {                                                                                                                                  #      et si c'est bien une valeur numerique  :
          ...
          slider1 <- tk2scale(Envir$Select, from=min(Envir$Data$Salinity, na.rm=TRUE), to=max(Envir$Data$Salinity, na.rm=TRUE),        # slider coulissant entre... 
                     ...
                     tkconfigure(Label1,text=paste(" Salinity min :", format(round((as.numeric(tclvalue(sal1))*2),0)/2, nsmall=1), "psu ")) })                      # titre des sliders
          Label1 <- tklabel(Envir$Select,text=paste(" Salinity min :", format(round((as.numeric(tclvalue(sal1))*2),0)/2, nsmall=1), "psu "))
          ...
          slider2 <- tk2scale(Envir$Select, from=min(Envir$Data$Salinity, na.rm=TRUE), to=max(Envir$Data$Salinity, na.rm=TRUE), 
                     ...
                     tkconfigure(Label2,text=paste(" Salinity max :", format(round((as.numeric(tclvalue(sal2))*2),0)/2, nsmall=1), "psu ")) })
          Label2 <- tklabel(Envir$Select,text=paste(" Salinity max :", format(round((as.numeric(tclvalue(sal2))*2),0)/2, nsmall=1), "psu "))
     ...
     if (any(colnames(Envir$Data) == "Depth") & any(!is.na(Envir$Data$Depth))==TRUE ) {                                                               # idem pour la profondeur
       if (class(Envir$Data$Depth) == "numeric" ) {
          ...
          slider3 <- tk2scale(Envir$Select, from=min(Envir$Data$Depth, na.rm=TRUE), to=max(Envir$Data$Depth, na.rm=TRUE),
                     ...
                     tkconfigure(Label3,text=paste("Depth min :", format(round((as.numeric(tclvalue(depth1))*2),0)/2, nsmall=1), "m")) })
          Label3 <- tklabel(Envir$Select,text=paste("Depth min :", format(round((as.numeric(tclvalue(depth1))*2),0)/2, nsmall=1), "m"))
          ...
          slider4 <- tk2scale(Envir$Select, from=min(Envir$Data$Depth, na.rm=TRUE), to=max(Envir$Data$Depth, na.rm=TRUE),
                     ...
                     tkconfigure(Label4,text=paste("Depth max :", format(round((as.numeric(tclvalue(depth2))*2),0)/2, nsmall=1), "m")) })
          Label4 <- tklabel(Envir$Select,text=paste("Depth max :", format(round((as.numeric(tclvalue(depth2))*2),0)/2, nsmall=1), "m"))
          ...
          tkgrid(slider4, column=0, row=24)  }
                         ...
                         , icon = "warning", type = "ok", title="!Warning!") } }
     else{ tkgrid(tklabel(Envir$Select, text="                                  "), column=0, row=21)
            tkgrid(tklabel(Envir$Select, text="  No depth values in  "), column=0, row=22)
            tkgrid(tklabel(Envir$Select, text="    the data frame    "), column=0, row=23)
            tkgrid(tklabel(Envir$Select, text="                                  "), column=0, row=24)  }
...
#_ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _champ separateur
...
#_______________________________________________________________________________bouton d'appel de la fonction 'select' dans FULLoption
     STAT2 <- function () {                                                                                                                # on rentre tout les arguments precedemment selectionne dans la fonction STAT2
          ...
          # if (cbVal=="1"){ mix <- "YES" }                                                              # agreger les category (obsolete)   
          # else { mix <- "NO" }
           mix <- "YES"
           if (any(colnames(Envir$Data) == "Salinity") & any(!is.na(Envir$Data$Salinity))==TRUE) {                     # valeurs de salinites
            Sal1 <- round((as.numeric(tclvalue(sal1))*2),0)/2
            Sal2 <- round((as.numeric(tclvalue(sal2))*2),0)/2
            sal <- c(Sal1, Sal2) }
           else{ sal <- "NULL" }
           if (any(colnames(Envir$Data) == "Depth") & any(!is.na(Envir$Data$Depth)) == TRUE) {                               # valeurs de profondeurs
            Depth1 <- round((as.numeric(tclvalue(depth1))*2),0)/2
            Depth2 <- round((as.numeric(tclvalue(depth2))*2),0)/2
...
#_______________________________________________________________________________fin du bouton
     ...
     HELP2.but <- tk2button(Envir$Select, image=imgHelp, text=" Help ", compound="right",command=function() { Aide2() } )     # bouton d'aide n°2  
...
#_______________________________________________________________________________fin du bouton
     ...
     tkgrid(LabeledFrame2, column=0, row=2, sticky="w")
         ...
         tkgrid(tklabel(LabeledFrame2,text="Monomonthly (see help)"),rb5, sticky="w")
...
#_______________________________________________________________________________fin du bouton      
     ...
     HELP3.but <- tk2button(Envir$datam, image=imgHelp, text=" Help ", compound="right", command=function() { Aide3() })       # bouton d'aide n°3
...
#_______________________________________________________________________________fin du bouton      
     ...
     HELP4.but <- tk2button(Envir$trend, image=imgHelp, text=" Help ", compound="right", command=function() {  Aide4() })      # bouton d'aide n°4
     tkgrid(HELP4.but, column=0, row=7, sticky="w")
   
   }
#_______________________________________________________________________________fin de la boucle d'ouverture du fichier (fin de copie-colle pour les fichiers fixdata.R et selectdirectory.R)  
...
#_________________________________________________________________________________________ce qui suit est affiche avant que les donnees ne soit importees
    ...
    Text3 <- tklabel(Envir$VersionFrame, text= paste("Need R v3.0.0+ to work properly; Your version:", getRversion() ))         # texte montrant la version minimum de R
...
#_______________________________________________________________________________________________________________________partie texte (resultats + messages) de l'interface
	...
	#Envir$pb.tta <- tk2progress(Envir$tt, orientation="horizontal", length=100)
	#tkpack(Envir$pb.tta, fill="x", expand=0, side="bottom")
	#tkconfigure(Envir$pb.tta)
